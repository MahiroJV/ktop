...
use crate::system::{SortBy, SystemStats};
use crossterm::{
    event::{
...
pub fn poll_event(state: &mut AppState) -> Action {
    ...
    match event::read() {
        ...
        Ok(Event::Key(k)) => match k.code {
            ...
            // sort shortcuts
            KeyCode::Char('c') => {
                state.sort_by = SortBy::CpuDesc;
                Action::Refresh
            }
            KeyCode::Char('C') => {
                state.sort_by = SortBy::CpuAsc;
                Action::Refresh
            }
            KeyCode::Char('m') => {
                state.sort_by = SortBy::MemDesc;
                Action::Refresh
            }
            KeyCode::Char('M') => {
                state.sort_by = SortBy::MemAsc;
                Action::Refresh
            }
            KeyCode::Char('p') => {
                state.sort_by = SortBy::Pid;
                Action::Refresh
            }
            KeyCode::Char('n') => {
...
pub fn draw(
    ...
    if area.width < 80 || area.height < 24 {
        let msg = Paragraph::new(Spans::from(vec![
            ...
            Span::styled(
                format!(
                    "terminal too small ({}x{}, need 80x24)",
...
fn draw_cpu(f: &mut Frame<CrosstermBackend<io::Stdout>>, area: Rect, s: &SystemStats) {
    ...
    f.render_widget(
        cpu_bar(s.cpu_total as u16, bar_w, "TOTAL", C_ACCENT, true),
        rows[0],
    );
    ...
    for (i, &usage) in s.cpu_cores.iter().take(8).enumerate() {
        ...
        f.render_widget(
            cpu_bar(usage as u16, core_w, &format!("C{}", i), color, false),
            rows[i + 2],
        );
    }
...
fn cpu_bar(
    ...
    Paragraph::new(Spans::from(vec![
        Span::styled(
            format!(" {:<6}", label),
            Style::default().fg(C_DIM).add_modifier(modifier),
        ),
        Span::styled("▐".to_string(), Style::default().fg(bar_color)),
        Span::styled(
            "█".repeat(filled),
            Style::default().fg(bar_color).add_modifier(modifier),
        ),
        Span::styled(
            "░".repeat(empty),
...
fn draw_memory(f: &mut Frame<CrosstermBackend<io::Stdout>>, area: Rect, s: &SystemStats) {
    ...
    f.render_widget(
        fancy_gauge(s.mem_percent as u16, "  RAM ", C_PURPLE),
        rows[0],
    );
    f.render_widget(
        stat_line(
    ...
    f.render_widget(
        fancy_gauge(s.swap_percent as u16, "  SWAP", C_ACCENT),
        rows[3],
    );
    f.render_widget(
        stat_line(
        ...
        .map(|(i, p)| {
            ...
            Row::new(vec![
                ...
                Cell::from(format!("{:.1}%", p.cpu)).style(
                    Style::default().fg(cpu_color).add_modifier(if is_selected {
                        Modifier::BOLD
                    } else {
                        Modifier::empty()
                    }),
...
fn draw_sort_bar(f: &mut Frame<CrosstermBackend<io::Stdout>>, area: Rect, state: &mut AppState) {
    ...
    for (label, sort) in buttons {
        ...
        f.render_widget(
            Paragraph::new(Spans::from(Span::styled(text, style))),
            btn_rect,
        );
...
fn draw_disk(f: &mut Frame<CrosstermBackend<io::Stdout>>, area: Rect, s: &SystemStats) {
    ...
    let bar_w = inner.width.saturating_sub(14) as usize;
    f.render_widget(
        cpu_bar(s.disk_percent as u16, bar_w, "/", C_GREEN, true),
        rows[0],
    );
    f.render_widget(
        stat_line(
            "  ◇",
            &format!("{:.1} GB / {:.1} GB", s.disk_used_gb, s.disk_total_gb),
            C_GREEN,
        ),
        rows[1],
    );
...
fn clip_path(path: &str, max: usize) -> String {
    ...
    if last.len() <= max {
        ...
        format!("/{}…", &last[..max.saturating_sub(1)])
    }
}
...
fn draw_network(f: &mut Frame<CrosstermBackend<io::Stdout>>, area: Rect, s: &SystemStats) {
    ...
    f.render_widget(
        Paragraph::new(Spans::from(vec![
            ...
            Span::styled(
                format!("{} MB", s.net_rx_total_mb),
                Style::default().fg(C_WHITE),
            ),
            Span::styled("  ↑ ", Style::default().fg(C_DIM)),
            Span::styled(
                format!("{} MB", s.net_tx_total_mb),
                Style::default().fg(C_WHITE),
            ),
        ])),
        rows[1],
